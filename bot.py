...
COMMON_WORD_ENDINGS = (
    ...
    "ily", "ily", "eed", "ing", "ed", "er", "es", "ly", "al", "ic",
    ...
    async def _generate(self, contents: list[types.Content]) -> str:
        ...
        for model_name in self.models:
            try:
                ...
                last_error = exc
                logger.warning("Model error %s: %s", model_name, exc, exc_info=True)

                if self._should_try_next_model(exc):
                    continue
                break

        logger.error("All models unavailable. Last error: %r", last_error)

        error_message = str(last_error or "").lower()
        if "timeout" in error_message or "deadline exceeded" in error_message:
            return (
                "The response took too long and the request timed out. "
                "Try shortening your request or breaking it into smaller parts."
            )

        return (
            "Could not get a response from the model right now. "
...
async def configure_webhook_with_retry(attempts: int = 10, delay_seconds: int = 6) -> bool:
    for attempt in range(1, attempts + 1):
        try:
            ...
            logger.warning(
                "Failed to register webhook, attempt %s/%s: %s",
                attempt,
                attempts,
                exc,
            )
    ...
    logger.error("Webhook could not be registered automatically. Bot is running and waiting for retry.")
    return False
...
async def disable_webhook_for_polling() -> None:
    try:
        ...
        logger.info("Webhook disabled. Bot is running in polling mode.")
    except Exception as exc:
...
async def start_handler(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "Hello! You can send me text messages or photos and I will respond using Google Gemini AI. "
        "The buttons below switch between modes. "
        "To clear the conversation memory, use /new.",
...
async def cancel_handler(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        "If you want to start fresh, use /new.",
        reply_markup=main_menu(),
    )
...
async def photo_mode_handler(message: Message, state: FSMContext) -> None:
    await state.set_state(UserState.waiting_photo)
    await message.answer(
        "Photo mode enabled. Send a photo or a follow-up question about the last photo."
    )
...
async def photo_message_handler(message: Message, state: FSMContext) -> None:
    try:
        ...
        if not isinstance(file_buffer, BytesIO):
            await message.answer("Could not download the photo.")
            return
...
async def text_message_handler(message: Message, state: FSMContext) -> None:
    ...
    try:
        if routing.reset_context:
            logger.info("New topic detected chat_id=%s, clearing previous context.", message.chat.id)
            await conversation_memory.clear_chat(message.chat.id)
            image_context_store.clear(message.chat.id)
            await state.set_state(UserState.waiting_text)
        ...
        await message.answer(
            ...
            "If your request is very long, try breaking it into smaller parts."
        )
...
async def fallback_handler(message: Message) -> None:
    await message.answer(
        "I understand text and photos. Please send a message or an image.",
        reply_markup=main_menu(),
    )
